class User < ActiveRecord::Base
  ...
  def self.top_rated
    self.select('users.*') # Select all attributes of a user
    ...
    .select('COUNT(DISTINCT comments.id) + COUNT(DISTINCT posts.id) AS rank') # Add the comment count and add it to the                 posts count as 'rank'
